...
(defun coq-smie-:=-deambiguate ()
  (let* (;; (orig (point))
         ...
         (corresp (coq-smie-search-token-backward
                   '("let" "Inductive" "CoInductive" "{|" "." "with" "Module" "where" "Equations" "fix")
                   cmdstrt
                   '((("let" "with") . ":=")))))
    (cond
     ((equal corresp "Equations") ":= equations")
     ((equal corresp "with")
...
(defun coq-smie-backward-token-aux ()
  (let* ((tok (smie-default-backward-token)))
    (cond
     ...
     ((equal tok "by")
      (save-excursion (coq-smie-by-deambiguate)))

     ((equal tok "as")
      (save-excursion (coq-smie-as-deambiguate)))
   ...
   (smie-bnf->prec2
    '((ident)
      (exp
       ...
       ("Com start" exp)
       ;;("Com start" exp proved-by-exps as-morphism-exp)
       ;;("Com start" exp with-signature-exp as-morphism-exp)
       )

      ;;(with-signature-exp ("with signature" exp))
      ;;(proved-by-exp ("xxx provedby" exp))
      ;;(as-morphism-exp ("as morphism" exp))
        ...
        (`(:before . ,_) ; indent token at point itself
         (let* ((cat (coq-indent-categorize-token-before token))
           ...
           (pcase cat
             ...
             ((and (or "with signature" "xxx provedby" "as morphism") (guard (parent-p "Com start")))
              (parent 2))
